...
2. `main` 브랜치에 푸시하면 `.github/workflows/preview.yml` 이 실행됩니다.
...
소스코드는 [MIT 라이선스](LICENSE)를 **제안**합니다. 공공기관이 제약 없이 재사용할 수 있는 형태입니다.
최종 라이선스는 충주시 담당부서 확인 후 확정해 주세요. 대안은 `LICENSE` 파일 하단을 참고하세요.
